...
func (_q *AgentQuery) QueryAuditRecords() *AuditRecordQuery {
...
func (_q *AgentQuery) QueryFeature() *FeatureQuery {
...
func (_q *AgentQuery) WithAuditRecords(opts ...func(*AuditRecordQuery)) *AgentQuery {
	query := (&AuditRecordClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AgentQuery) WithFeature(opts ...func(*FeatureQuery)) *AgentQuery {
	query := (&FeatureClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AgentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Agent, error) {
	...
	if query := _q.withAuditRecords; query != nil {
		if err := _q.loadAuditRecords(ctx, query, nodes,
			func(n *Agent) { n.Edges.AuditRecords = []*AuditRecord{} },
			func(n *Agent, e *AuditRecord) { n.Edges.AuditRecords = append(n.Edges.AuditRecords, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withFeature; query != nil {
		if err := _q.loadFeature(ctx, query, nodes,
			func(n *Agent) { n.Edges.Feature = []*Feature{} },
			func(n *Agent, e *Feature) { n.Edges.Feature = append(n.Edges.Feature, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AgentQuery) loadAuditRecords(ctx context.Context, query *AuditRecordQuery, nodes []*Agent, init func(*Agent), assign func(*Agent, *AuditRecord)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(auditrecord.FieldAgentID)
	}
	query.Where(predicate.AuditRecord(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(agent.AuditRecordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *AgentQuery) loadFeature(ctx context.Context, query *FeatureQuery, nodes []*Agent, init func(*Agent), assign func(*Agent, *Feature)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Agent)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Feature(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(agent.FeatureColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.agent_feature
		if fk == nil {
			return fmt.Errorf(`foreign-key "agent_feature" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "agent_feature" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
