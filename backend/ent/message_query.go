...
func (_q *MessageQuery) QueryAction() *ActionModelQuery {
...
func (_q *MessageQuery) WithAction(opts ...func(*ActionModelQuery)) *MessageQuery {
	query := (&ActionModelClient{config: _q.config}).Query()
...
func (_q *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if query := _q.withAction; query != nil {
		if err := _q.loadAction(ctx, query, nodes, nil,
			func(n *Message, e *ActionModel) { n.Edges.Action = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MessageQuery) loadAction(ctx context.Context, query *ActionModelQuery, nodes []*Message, init func(*Message), assign func(*Message, *ActionModel)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Message)
	for i := range nodes {
		if nodes[i].message_action == nil {
			continue
		}
		fk := *nodes[i].message_action
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "message_action" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
